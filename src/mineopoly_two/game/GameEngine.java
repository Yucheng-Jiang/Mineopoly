...
public class GameEngine extends Observable {
    ...
    private void setupEngineForGame(int boardSize, long randomSeed) {
        ...
        // Reflection hackery to set a seed value on Math.random()
        for (Class<?> mathSubclass : Math.class.getDeclaredClasses()) {
            if (mathSubclass.getSimpleName().equals("RandomNumberGeneratorHolder")) {
                // Subclass that holds the random number generator located
                try {
                    // Specify which field we're getting
                    Field mathRNGField = mathSubclass.getDeclaredField("randomNumberGenerator");
                    // Set the field to public so we can mess with it (it's currently package-private)
                    mathRNGField.setAccessible(true);
                    // Actually get the value of the field, since it's static we don't need to pass in an Object
                    Random mathClassRNG = (Random) mathRNGField.get(null);
                    mathClassRNG.setSeed(randomSeed);
                } catch (NoSuchFieldException e) {
                    System.err.println("Could not find randomNumberGenerator field, are you on Java 8?");
                } catch (IllegalAccessException e) {
                    System.err.println("Could not access randomNumberGenerator field, are you on Java 8?");
                }
                break;
            }
        }
    ...
    public void initializePlayer(MinePlayer playerToInitialize, boolean isRedPlayer) {
    ...
    public void processTurn(MinePlayer currentPlayer, MinePlayer otherPlayer, boolean isRedTurn) {
        ...
        try {
            playerAction = playerStrategy.getTurnAction(boardView, economy, robotCharge, isRedTurn);
        } catch (IOException e) {
            playerAction = null;
        }
